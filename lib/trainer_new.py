...
class TrainerStep():
    def __init__(self, pinn, ckpt_dir, ckpt_interval=100, device=torch.device('cpu'), logger=logging.getLogger(__name__)):
        self.pinn = pinn
        self.device = device
        self.logger = logger
    ...
    def divide_epochs(self, mode:str, epochs:int, steps:int):
        if mode == 'linear':
    ...
    def validate(self, test_data, errors, ckpt, ckpt_path):
        self.pinn.eval()
        with torch.no_grad():
        ...
        if ckpt and mae < self.min_error:
            self.min_error = mae
    ...
    def train_epoch(self, bulk_data, bdry_data, init_data, optimizer, scheduler, weights):
        optimizer.zero_grad()
        ...
        return total_loss, bulk_loss, boundary_loss, initial_loss
    ...
    def train(self,
        ...
        # Decomposition of the Epochs
        decomposition_epochs = self.divide_epochs(divide_mode, epochs, steps)
        ...
        for step in range(steps):
            ...
            try:
                ...
                raise RuntimeError("Error in dividing the data") from e
            ...
            for epoch in range(epoch_start, epoch_end):
                ...
                if not profiling_done:
                    with torch.profiler.profile(
                        ...
                        total_loss, bulk_loss, boundary_loss, initial_loss = self.train_epoch(self, bulk_data_temp, bdry_data, init_data, optimizer, scheduler, weights)
                ...
                if ckpt and (epoch % self.ckpt_interval == 0 or epoch == decomposition_epochs[-1] - 1):
                    
                    mae = self.validate(test_data, errors, ckpt, ckpt_path)
...
class TrainerStepNTK(TrainerStep):
    ...
    def train_epoch(self, bulk_data, bdry_data, init_data, optimizer, scheduler, weights, ntk=False):
        optimizer.zero_grad()
            ...
            for epoch in range(epoch_start, epoch_end):
                ...
                if not profiling_done:
                    with torch.profiler.profile(
                        ...
                        total_loss, bulk_loss, boundary_loss, initial_loss = self.train_epoch(self, bulk_data_temp, bdry_data, init_data, optimizer, scheduler, weights, ntk=True)
...
class TrainerStepGradientGIF(TrainerStep):

    def validate(self, plotter, image_dir, epoch, bulk_data, weights, boundary_loss, initial_loss, errors, ckpt, ckpt_path):
        self.pinn.eval()
        bulk_loss = self.pinn.bulk_loss(bulk_data)
        total_loss = weights.get('bulk', 1.0) * bulk_loss + \
        ...
        if ckpt and total_loss.item() < self.min_loss:
            self.min_loss = total_loss.item()
            torch.save(self.pinn.state_dict(), ckpt_path)

    def train_epoch(self, plotter, bulk_data, bdry_data, init_data, optimizer, scheduler, weights, ntk=False):
        optimizer.zero_grad()
    ...
    def train(self,
        ...
        for step in range(steps):
            ...
            for epoch in range(epoch_start, epoch_end):
                ...
                # Epoch
                total_loss, bulk_loss, boundary_loss, initial_loss = self.train_epoch(self, plotter, bulk_data_temp, bdry_data, init_data, optimizer, scheduler, weights)
                
                # Logging and Validation
                if ckpt and (epoch % self.ckpt_interval == 0 or epoch == decomposition_epochs[-1] - 1):
                    
                    self.validate(plotter, image_dir, epoch, bulk_data, weights, boundary_loss, initial_loss, errors, ckpt, ckpt_path)
